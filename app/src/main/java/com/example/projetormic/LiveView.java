...
public class LiveView extends AppCompatActivity implements ArrayFetch.DataFetchedListener {
    ...
    private Runnable fetchDataRunnable = new Runnable() {
        ...
        public void run() {
            ...
            ArrayFetch.fetchDataFromFirebase("solar_panel_data/thermal_frame", (data) -> {
                onDataFetched(data);
            });

            Log.d("autoDataFetch", "autoDataFetch completed");

            // Repeat the task after 5 seconds
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        openedAutomatically = getIntent().getBooleanExtra("openedAutomatically", false);
        ...
        autoCameraRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    if (!autoCameraEnabled && openedAutomatically) {
                        Log.d("LiveFeed", "thermal_camera_auto is false and activity was opened automatically, closing LiveFeed.");
                        finish(); // Close LiveFeed activity only if opened automatically
                    }
                }
            }
        ...
        databaseRef.onDisconnect().setValue(false).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Log.d("FirebaseUpdate", "enable_thermal_camera_view will be set to false on disconnect");
            } else {
                Log.e("FirebaseUpdate", "Failed to set onDisconnect for enable_thermal_camera_view", task.getException());
            }
        });
        ...
        handler.post(fetchDataRunnable);
            Log.d("autoDataFetch", "autoDataFetch started");
    ...
    private void fetchDataFromFirebase() {
        ...
        DatabaseReference avgTempRef = FirebaseDatabase.getInstance().getReference("solar_panel_data/baseline_temp");
        ...
        DatabaseReference illuminanceRef = FirebaseDatabase.getInstance().getReference("solar_panel_data/lux");
    ...
    public void onDataFetched(double[][] data) {
        // Handle the fetched data and update the heatmap
    ...
    protected void onStop() {
        ...
        // Stop the periodic task when activity is stopped
        handler.removeCallbacks(fetchDataRunnable);
            Log.d("autoDataFetch", "autoDataFetch stopped");

        // Get Firebase database reference
        DatabaseReference databaseRef = FirebaseDatabase.getInstance()
                .getReference("enable_thermal_camera_view");

        // Update the value to false
        databaseRef.setValue(false).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Log.d("FirebaseUpdate", "enable_thermal_camera_view set to false");
            } else {
